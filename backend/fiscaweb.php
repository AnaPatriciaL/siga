...
$updateEnvio = $conexion->prepare("UPDATE siga_prospectos_ordenes SET envio_fiscaweb = 1 WHERE num_orden = :num_orden AND anio = :anio AND (envio_fiscaweb = 0 OR ISNULL(envio_fiscaweb))");
    ...
    case 1: // CONSULTAR 
        ...
        try {
            ...
            CASE
                ...
                ELSE ''
            END AS EJE2_EJERCICIO,UPPER(pr.nombre_completo) AS PROGRAMADOR_RFC, '' AS PORTALUSER, '' AS FECALTASIS, '' AS ESTATUS_ESTATUS,o.periodos AS PERIODO
            ...
            WHERE o.estatus = 1 AND (o.envio_fiscaweb = 0 OR ISNULL(o.envio_fiscaweb)) ORDER BY o.num_orden";
            $resultado = $conexion->prepare($consulta);
            $resultado->execute();
            $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
            foreach ($data as &$row) {
                $numOrden = $row['ORDEN'];
                $fechaOrden = $row['FECORDEN'];
                $rfc = $row['RFC'];
                ...
                $updateEnvio->execute([':num_orden' => $numOrden, ':anio' => $anio]);
            }
            unset($row);
            $conexion->commit();
